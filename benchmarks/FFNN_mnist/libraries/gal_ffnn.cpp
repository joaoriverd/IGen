...
/* Auxiliary arrays */
static gal_I _y1[MAX_LAYER_SIZE];
static gal_I _y2[MAX_LAYER_SIZE];

static void WeightedSum_opt(gal_I* W, gal_I* b, gal_I* x, gal_I* y, int n_in, int num_n) {
    /* Handle remaining */
    for (int i = 0; i < num_n; i+=2) {
        gal_I y00 = 0.0, y01 = 0.0, y02 = 0.0, y03 = 0.0;
        gal_I y10 = 0.0, y11 = 0.0, y12 = 0.0, y13 = 0.0;
...
static void WeightedSum(gal_I* W, gal_I* b, gal_I* x, gal_I* y, int n_in, int num_n) {
...
static void sigmoid_exp(gal_I* y, int n) {
    for (int i = 0; i < n; ++i) {
        y[i] = y[i] / (1.0 + exp(-y[i]*5.0));
    }
}

static void sigmoid_relu(gal_I* y, int n) {
    for (int i = 0; i < n; ++i) {
        y[i] = y[i] > 0.0 ? y[i] : 0.0;
    }
}

static void processLayer(gal_I* W, gal_I* b, gal_I* x, gal_I* y, int n_in, int num_n) {
...
void FFNN(gal_I** W, gal_I**b, gal_I* x, gal_I* y, int n_in, int n_out, int num_l,  int num_n) {
    gal_I* ptr_y1 = _y1;
    gal_I* ptr_y2 = _y2;
