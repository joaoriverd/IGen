...
/* Auxiliary arrays */
static fil_I _y1[MAX_LAYER_SIZE];
static fil_I _y2[MAX_LAYER_SIZE];

static void WeightedSum_opt(fil_I* W, fil_I* b, fil_I* x, fil_I* y, int n_in, int num_n) {
    /* Handle remaining */
    for (int i = 0; i < num_n; i+=2) {
        fil_I y00 = 0.0, y01 = 0.0, y02 = 0.0, y03 = 0.0;
        fil_I y10 = 0.0, y11 = 0.0, y12 = 0.0, y13 = 0.0;
...
static void WeightedSum(fil_I* W, fil_I* b, fil_I* x, fil_I* y, int n_in, int num_n) {
...
static void sigmoid_exp(fil_I* y, int n) {
    for (int i = 0; i < n; ++i) {
        y[i] = y[i] / (1.0 + exp(-y[i]*5.0));
    }
}

static void sigmoid_relu(fil_I* y, int n) {
    for (int i = 0; i < n; ++i) {
//        y[i] = fmax(0.0, y[i]);
//        y[i] = y[i] > 0.0 ? y[i] : 0.0;
    }
...
static void processLayer(fil_I* W, fil_I* b, fil_I* x, fil_I* y, int n_in, int num_n) {
...
void FFNN(fil_I** W, fil_I**b, fil_I* x, fil_I* y, int n_in, int n_out, int num_l,  int num_n) {
    fil_I* ptr_y1 = _y1;
    fil_I* ptr_y2 = _y2;
