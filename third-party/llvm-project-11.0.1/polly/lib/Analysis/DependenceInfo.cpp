  ...
  //    write and before the first access to a reduction location).
    ...
    for (ScopStmt &Stmt : S) {
        for (MemoryAccess *MA : Stmt) {
            if (!MA->isReductionLike())
                continue;
            isl_union_map *CurrentRED = isl_union_map_empty(isl_union_map_get_space(RAW));
            ...
            CurrentRED = isl_union_map_add_map(CurrentRED, Identity);
            CurrentRED = isl_union_map_intersect(CurrentRED, isl_union_map_copy(RAW));
            CurrentRED = isl_union_map_intersect(CurrentRED, isl_union_map_copy(StrictWAW));
            CurrentRED = isl_union_map_zip(CurrentRED);
            CurrentRED = isl_union_set_unwrap(isl_union_map_domain(CurrentRED));
            CurrentRED = isl_union_map_coalesce(CurrentRED);

            if (!isl_union_map_is_empty(CurrentRED)) {
                if (MA->isRead()) {;
                    auto  *LoadInstruction = dyn_cast<LoadInst>(MA->getAccessInstruction());
                    auto *StoreInstruction = dyn_cast<StoreInst>(MA->storeInReductionLike->getAccessInstruction());
                    unsigned LoadLine  = LoadInstruction->getDebugLoc().getLine();
                    unsigned LoadCol   = LoadInstruction->getDebugLoc().getCol();
                    unsigned StoreLine = StoreInstruction->getDebugLoc().getLine();
                    unsigned StoreCol  = StoreInstruction->getDebugLoc().getCol();

                    /* Construct mapping with current domain */
                    isl::set domain = Stmt.getDomain();
                    isl::map fullMap  = isl::map::from_domain_and_range(domain, domain); // Map the current domain

                    /* Get dimension and space */
                    int dim_in   = fullMap.dim(isl::dim::in);
                    isl::space s = fullMap.get_space();

                    /* Find loop level of reduction */
                    int loopLvl = 0;
                    for (int lvl = 1; lvl <= dim_in; lvl++) {
                        /* Construct mapping in lexicographic order for the lower dimensions only  */
                        isl::map t1;
                        if (lvl < dim_in) {
                            isl::map t2  = isl::map::lex_lt_first(s, dim_in);
                            isl::map t3  = isl::map::lex_lt_first(s, dim_in - lvl);
                            t1 = t2.subtract(t3);
                        }
                        else {
                            t1 = isl::map::lex_lt(s.domain());
                        }

                        isl::map lexOrderMap = t1.intersect(fullMap);

                        /* Use the let min to prune the relations (only keep the next of the lower dimensions) */
                        isl::union_map loopLvlMap = lexOrderMap.lexmin();

                        /* Check if the current reduction holds for the first level of the loop */
                        isl::union_map wrapCurrentRed = isl::manage_copy(CurrentRED);

                        // std::cerr << dim_in << std::endl;
                        // std::cerr << fullMap.to_str()     << "\n";
                        // std::cerr << lexOrderMap.to_str() << "\n";
                        // std::cerr << loopLvlMap.to_str()  << "\n\n";

                        if (loopLvlMap.is_subset(wrapCurrentRed)) {
                            loopLvl = lvl;
                        }
                        else {
                            /* Reduction does not hold in this level. It will not hold in upper levels either */
                            break;
                        }
                    }
                    ...
                    if (loopLvl > 0) {
                        /* Format output */
                        static bool headerFlag = true;
                        if (headerFlag) {
                            /* Print header the first time only */
                            std::cerr << "LineLoad,ColnLoad,LineStore,ColnStore,LoopLvl\n";
                            headerFlag = false;
                        }

                        std::cerr << LoadLine  << "," << LoadCol  << ",";
                        std::cerr << StoreLine << "," << StoreCol << ",";
                        std::cerr << loopLvl << "\n";
                    }
                }
            }
            isl_union_map_free(CurrentRED);
        }
    }
