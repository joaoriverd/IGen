...
static MemoryAccess::ReductionType getReductionType(const BinaryOperator *BinOp,
  ...
  case Instruction::FAdd:
    // JR: We treat all floats as fast (reals)
...
void ScopBuilder::checkForReductions(ScopStmt &Stmt) {
  ...
  for (const auto &CandidatePair : Candidates) {
    ...
    // Print instruction and save the store memory access in the load access
    MemoryAccess   *LoadMA = CandidatePair.first;
    MemoryAccess  *StoreMA = CandidatePair.second;

    // Save the store memory access in the load access
    LoadMA->storeInReductionLike = StoreMA;
  }
}
...
void ScopBuilder::collectCandidateReductionLoads(
  ...
  /* JR: Detect reductions only on additions */
