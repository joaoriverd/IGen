...
StmtResult
Parser::ParsePragmaIGen(StmtVector &Stmts,
                        ParsedStmtContext StmtCtx,
                        SourceLocation *TrailingElseLoc,
                        ParsedAttributesWithRange &Attrs) {
    // Create temporary attribute list.
    ParsedAttributesWithRange TempAttrs(AttrFactory);
    while (Tok.is(tok::annot_pragma_igen)) {
        IGenHint Hint;
        if (!HandlePragmaIGen(Hint))
            continue;
        if (Hint.OptionLoc->Ident->getName() == "reduce") {
...
Retry:
  ...
  case tok::annot_pragma_igen:
    ProhibitAttributes(Attrs);
    return ParsePragmaIGen(Stmts, StmtCtx, TrailingElseLoc, Attrs);
