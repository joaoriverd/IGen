...
struct PragmaIGenHandler : public PragmaHandler {
    explicit PragmaIGenHandler() : PragmaHandler("igen") {}
...
void Parser::initializePragmaHandlers() {
  IGenHandler = std::make_unique<PragmaIGenHandler>();
  PP.AddPragmaHandler(IGenHandler.get());
...
void Parser::resetPragmaHandlers() {
  ...
  PP.RemovePragmaHandler(IGenHandler.get());
  IGenHandler.reset();
...
static bool ParseIGenValue(Preprocessor &PP, Token &Tok, Token PragmaName,
                           Token Option, PragmaIGenInfo &Info) {
    SmallVector<Token, 1> ValueList;
    while (Tok.isNot(tok::eod)) {
        ValueList.push_back(Tok);
        PP.Lex(Tok);
...
void PragmaIGenHandler::HandlePragma(Preprocessor &PP,
                                     PragmaIntroducer Introducer,
                                     Token &Tok) {
    Token PragmaName = Tok;
    SmallVector<Token, 1> TokenList;
    PP.Lex(Tok);
    if (Tok.isNot(tok::identifier)) {
        printf("Error, not a identifier token for the option of pragma igen\n");
        return;
    }
    ...
    bool OptionValid = llvm::StringSwitch<bool>(OptionInfo->getName())
            ...
            .Default(false);
    if (!OptionValid) {
        printf("Error, option not recognized for pragma igen\n");
        return;
    }
    PP.Lex(Tok);

    auto *Info = new (PP.getPreprocessorAllocator()) PragmaIGenInfo;
    if (!ParseIGenValue(PP, Tok, PragmaName, Option, *Info)) {
        return;
    }

    Token IGenTok;
    IGenTok.startToken();
    IGenTok.setKind(tok::annot_pragma_igen);
    IGenTok.setLocation(PragmaName.getLocation());
    IGenTok.setAnnotationEndLoc(PragmaName.getLocation());
    IGenTok.setAnnotationValue(static_cast<void *>(Info));
    TokenList.push_back(IGenTok);

    if (Tok.isNot(tok::eod)) {
        printf("Error, extra tokens at the end of pragma igen\n");
        PP.Diag(Tok.getLocation(), diag::warn_pragma_extra_tokens_at_eol)
                << "igen pragma";
        return;
    }
    ...
    std::copy(TokenList.begin(), TokenList.end(), TokenArray.get());
    PP.EnterTokenStream(std::move(TokenArray), TokenList.size(),
            /*DisableMacroExpansion=*/false, /*IsReinject=*/false); //(JR)
}


bool Parser::HandlePragmaIGen(IGenHint &Hint) {
    assert(Tok.is(tok::annot_pragma_igen));
    PragmaIGenInfo *Info =
            static_cast<PragmaIGenInfo *>(Tok.getAnnotationValue());
    ...
    bool OptionReduce = OptionInfo->isStr("reduce");
    ...
    if (OptionReduce) {
        ...
        printf("No reduce as option parameters\n");
    }
    // Tokens following an error in an ill-formed constant expression will
    // remain in the token stream and must be removed.
    if (Tok.isNot(tok::eof)) {
