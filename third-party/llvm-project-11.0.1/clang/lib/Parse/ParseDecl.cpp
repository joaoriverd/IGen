  ...
  while (1) {
    ...
    case tok::kw_float:
      isInvalid = DS.SetTypeSpecType(DeclSpec::TST_float, Loc, PrevSpec,
                                     DiagID, Policy);
      // Language extension for tolerance declaration in type.
      // Consume two tokens to bypass declarations of the type float:1.0. (JR)
      if (GetLookAheadToken(1).is(tok::colon) && GetLookAheadToken(2).is(tok::numeric_constant)) {
          ConsumeToken();
          ConsumeToken();
      }
      break;
    case tok::kw_double:
      isInvalid = DS.SetTypeSpecType(DeclSpec::TST_double, Loc, PrevSpec,
                                     DiagID, Policy);
      // Language extension for tolerance declaration in type.
      // Consume two tokens to bypass declarations of the type double:1.0.
      if (GetLookAheadToken(1).is(tok::colon) && GetLookAheadToken(2).is(tok::numeric_constant)) {
          ConsumeToken();
          ConsumeToken();
      }
...
bool Parser::isTypeSpecifierQualifier() {
  ...
  case tok::kw_float:
  case tok::kw_double:
    if (Tok.getKind() == tok::kw_float || Tok.getKind() == tok::kw_double) {
        // Language extension for tolerance declaration in type.
        // Consume two tokens to bypass declarations of the type double:1.0.
        if (GetLookAheadToken(1).is(tok::colon) && GetLookAheadToken(2).is(tok::numeric_constant)) {
            ConsumeToken();
