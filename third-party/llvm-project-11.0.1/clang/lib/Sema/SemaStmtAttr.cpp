...
static Attr *handleIGenAttr(Sema &S, Stmt *St, const ParsedAttr &A,
                            SourceRange) {
    IdentifierLoc *PragmaNameLoc = A.getArgAsIdent(0);
    IdentifierLoc *OptionLoc = A.getArgAsIdent(1);
    Expr *ValueExpr = nullptr;
    bool PragmaReduce = OptionLoc->Ident->getName() == "reduce";
    IGenAttr::OptionType Option;
    if (PragmaReduce) {
        Option = IGenAttr::Reduce;
        ValueExpr = A.getArgAsExpr(2);
    } else {
        printf("Error, no reduce\n");
    }
    if (!ValueExpr) {
        printf("Error in Sema getting N\n");
    }
    return IGenAttr::CreateImplicit(S.Context, Option, ValueExpr, A.getRange());
}
...
static Attr *ProcessStmtAttribute(Sema &S, Stmt *St, const ParsedAttr &A,
  ...
  case ParsedAttr::AT_IGen:
    return handleIGenAttr(S, St, A, Range);
